...
let brojLetovaZaZemlju = (niz, drzava) => {
  let br = 0;
  niz.forEach((let) => {
    if (let.drzava == drzava) {
      br++;
    }
  });
  console.log(br);
};
brojLetovaZaZemlju(letovi, "Nemacka");
...
let visePrePodne = (niz) => {
  ...
  let prePodne = 0;
  niz.forEach((let) => {
    let vPoletanja = Number(let.vremePoletenja.split(":")[0]);
    // console.log(vPoletanja);
    if (vPoletanja < 12) {
      prePodne++;
    }
  });
...
let ispisLetovaDoSada = (niz, drzava) => {
  niz.forEach((let) => {
    if (let.drzava == drzava) {
      let satiLeta = Number(let.vremePoletenja.split(":")[0]);
      let minutiLeta = Number(let.vremePoletenja.split(":")[1]);
      //   console.log(satiLeta, minutiLeta);
      if (sati > satiLeta) {
        console.log(let);
      } else if (sati == satiLeta && minuti > minutiLeta) {
        console.log(let);
      }
    }
  });
};
ispisLetovaDoSada(letovi, "Spanija");
...
let rizicniLetovi = (niz, zemlje) => {
  let paragraf = document.createElement("p");
  niz.forEach((let) => {
    zemlje.forEach((zemlja) => {
      if (let.drzava == zemlja) {
        paragraf.innerHTML += `${let.destinacija} ${let.vremePoletenja} <br>`;
